...
var transBox = ['.work-one', '.work-two', '.work-three'];
...
function myScroll() {
	...
	try {
		for (var _iterator2 = elts[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
			...
			if (window.pageYOffset >= _elt.y && window.pageYOffset !== getScrollMaxY()) {
				_elt.a.classList.add('active');
				break;
			}
		}
	} catch (err) {
...
function showTransition() {
	...
	try {
		for (var _iterator3 = transBox[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
			var t = _step3.value;

			var elt = document.querySelector(t);
...
function parallaxImg() {
	var img = document.querySelector('.img-parallax');
	var parent = document.querySelector('#howItWorks');
...
function menu() {
	...
	var _arr = [].concat(_toConsumableArray(document.querySelectorAll('[data-name]')));
	...
	for (var _i = 0; _i < _arr.length; _i++) {
		...
		section = document.getElementById(_a.getAttribute('data-name'));
		_a.href = urlBase;
		elts.push({ a: _a, y: parseInt(section.getBoundingClientRect().top) - DOMheader.offsetHeight });
	}
	elts.reverse();
	myScroll();
}
...
document.querySelector('.arrow.next').addEventListener('click', function (evt) {
  currentCreaIndex++;
...
document.querySelector('.arrow.prev').addEventListener('click', function (evt) {
  currentCreaIndex--;
...
function selectCreation(creation) {
  ...
  try {
    for (var _iterator = creation.params[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var p = _step.value;
  ...
  Wb(creation.name).init('#crea > div', params).then(function (crea) {
    ...
    var _loop = function _loop(key, fn) {
      var elt = void 0;
      var isColor = false;
      switch (fn.type) {
        case 'color':
          var colorFn = function colorFn(jscolor) {
            c[fn.key]('#' + jscolor);
          };
          if (fn.nbParams == 2) {
            // Color main with its complementary
            colorFn = function colorFn(jscolor) {
    ...
    try {
      for (var _iterator2 = creation.functions.entries()[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
...
function buildParam(name, elt, isColor) {
  var main = document.querySelector('#crea-params');
